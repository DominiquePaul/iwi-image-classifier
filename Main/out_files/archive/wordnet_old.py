...
def extract_features_wordnet(label_dict, comparison_word):
    ...
    for label, value in label_dict.items():
        ...
        # generally the WUP score can only be positive. The only exception is
        # when the label cannot be found in the wordnet dictionary and thus a
        # a wup score of 0 returned, causing the label to be skipped
        if max_similarity > 0:
...
def identify_item(img, object_name, k_labels):
    """ a model that checks whether a group of words is contained in the
    the inception model predictions
    """
    item_synonyms = []
    for word in wordnet.synsets(object_name):
        item_synonyms.extend(word.lemma_names())
...
if __name__ == "__main__":
    ...
    out_file = 'out_files/wordnet_hyperopt_out.csv'
    ...
    for label_amount in n_label_list:
        ...
        for img in tqdm(x_test):
            predictions.extend([(identify_item(img, synonyms, k_labels=label_amount))])

        approach = "{} labels".format(label_amount)
